...
const TIMEFRAME = 300; // 5 MINUTOS
const MOMENTUM_THRESHOLD = 0.20;

const CONFIG = {
    LOOKBACK: 20,
    GRINDING_THRESHOLD: 0.08,
    TP_RATIO: 1.2,
    SL_BASE: 0.30,
    MIN_CANDLES: 35,
    MAX_CANDLES: 300,
...
function isGoodTime() {
    ...
    const goodHours = [2,3,4,8,9,10,14,15,16,20,21,22];
...
function hasGrindingConfirmation(candles, lookback = 10) {
...
function checkSpikeSignal(sym) {
    ...
    // 🔥 SIN FILTRO DE PROBABILIDAD - CUALQUIER PROBABILIDAD GENERA SEÑAL
...
function analyzeSignal(sym) {
        ...
        if (!st.lastSignal || st.signalExpired) {
            ...
            if (signal) {
                ...
                const msg = `KRAKEN PRO - SEÑAL\n\n` +
                    ...
                    `Hora: ${signal.time}\n\n` +
                    `KRAKEN PRO - SIN FILTRO DE PROBABILIDAD\nMONITOREO EN TIEMPO REAL\n1 OPERACION POR PAR`;
    ...
    } catch (e) { addLog(`Error en ${sym}: ${e.message}`, 'error'); }
...
function openWS(url) {
    ...
    ws.onclose = () => { addLog('WebSocket cerrado', 'warn'); if (running) scheduleReconnect(); };
    ws.onerror = () => {};
...
function scheduleReconnect() {
    ...
    if (reconnectAttempts >= 20) { addLog('Max reintentos', 'error'); return; }
    reconnectAttempts++;
...
function openWS(url) {
    ...
    ws.onopen = () => {
        ...
        setTimeout(() => {
            ...
            if (!activationSent) {
                activationSent = true;
                sendTelegramMessage(`KRAKEN PRO ACTIVADO\n\nBot conectado\nMonitoreando BOOM500, BOOM600, BOOM900, BOOM1000\nProbabilidad minima: 92%\nMULTIPLES FILTROS DE PRECISION\nTemporalidad: 1 minuto\nMONITOREO EN TIEMPO REAL\n1 OPERACION POR PAR\nMAXIMA PRECISION\nEsperando señales...`);
            }
        }, 5000);
    };
    ws.onclose = () => { addLog('WebSocket cerrado', 'warn'); if (running) scheduleReconnect(); };
...
console.log('KRAKEN PRO - 24/7 ACTIVO');
console.log('MULTIPLES FILTROS DE PRECISION');

addLog('Iniciando KRAKEN PRO (MULTIPLES FILTROS)...', 'info');

setTimeout(() => {
    sendTelegramMessage(`KRAKEN PRO INICIADO\n\nConectando a Deriv...\nEl bot se activara automaticamente\n${ALL_PAIRS.length} simbolos\nProbabilidad minima: ${CONFIG.MIN_PROBABILITY}%\nMULTIPLES FILTROS DE PRECISION\nTemporalidad: ${TIMEFRAME/60} min\n1 OPERACION POR PAR\nMONITOREO EN TIEMPO REAL\nMAXIMA PRECISION\n${new Date().toLocaleString()}`);
}, 3000);
