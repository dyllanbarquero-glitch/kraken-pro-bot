...
console.log('📊 ESTRATEGIA: MERCADO EN MOVIMIENTO');
...
const CONFIG = {
    MIN_PROBABILITY: 80,
    LOOKBACK: 30,
    TREND_THRESHOLD: 1.0,        // Rango mínimo para considerar tendencia
    MOMENTUM_TREND: 0.30,        // Momentum mínimo para considerar tendencia
    TP_RATIO: 1.0,
    SL_BASE: 0.15,
    MIN_CANDLES: 30,
    MAX_CANDLES: 500
...
function detectTrend(sym) {
    ...
    // Tendencia: rango > 1% y momentum > 0.30%
...
// 🔥 CALCULAR PROBABILIDAD DE TENDENCIA
...
function checkSignal(sym) {
    ...
    if (probability >= minProb && !st._signalSent && !st._signalClosed) {
        const price = st.price;
        const result = calculateTPSL(price, st.candles, true);
        const signal = {
            sym, type: 'MULTUP', price,
        ...
        addLog(`🔔 ${sym}: 📈 SEÑAL TENDENCIA | Prob: ${probability}% | Entry: $${price.toFixed(4)} | TP: $${signal.tp1.toFixed(4)} | SL: $${signal.sl.toFixed(4)}`, 'signal');
        return signal;
    }
    return null;
}
...
connectDeriv();let botStats = { balance: 0, totalProfit: 0, winCount: 0, lossCount: 0, totalTrades: 0 };
let reconnectAttempts = 0;
let reconnectTimer = null;
let lastCandleKey = {};
let candleCloseProcessed = {};
